...
class Qiniu implements UploadInterface{
	...
	public function save(&$file, $replace = true){
		$key = $file['save_path'] . $file['save_name'];
		$upfile = array(
			'name' => 'file',
			'fileName' => $key,
...
class QiniuStorage{
	...
	public function upload($config, $file){
		...
		$fields = array(
			'token' => $uploadToken,
			'key' => !empty($config['save_name']) ? $config['save_name'] : $file['fileName'],
		);

		if(isset($config['custom_fields']) && isset($config['custom_fields']) && is_array($config['custom_fields']) && $config['custom_fields'] !== array()){
